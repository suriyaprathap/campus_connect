...
export const addAdmin = async (req, res) => {
  try {
    ...
    let departmentHelper = existingDepartment.departmentCode; // Extract MCA

    // Count existing admins in the department
    const adminCount = await Admin.countDocuments({ department }) + 1; // Start from 1
    const helper = String(adminCount).padStart(3, "0"); // Ensures 001, 002, etc.

    var date = new Date();
    var components = ["ADM", date.getFullYear().toString().slice(-2), departmentHelper, helper];
    var username = components.join("");  // Generates ADM25MCA001
...
export const addFaculty = async (req, res) => {
  try {
    const { name, dob, department, contactNumber, avatar, email, joiningYear, gender, designation } = req.body;
    ...
    // Count existing faculty members in the department
    const facultyCount = await Faculty.countDocuments({ department }) + 1;
    const helper = String(facultyCount).padStart(3, "0");

    var date = new Date();
    var components = ["FAC", date.getFullYear().toString().slice(-2), departmentHelper, helper];
    var username = components.join(""); // Generates FAC25MCA001
...
export const addStudent = async (req, res) => {
  try {
    const { name, dob, department, contactNumber, avatar, email, section, gender, batch, fatherName, motherName, fatherContactNumber, motherContactNumber, year } = req.body;
    ...
    // Count existing students in the department
    const studentCount = await Student.countDocuments({ department }) + 1;
    const helper = String(studentCount).padStart(3, "0");

    var date = new Date();
    var components = ["STU", date.getFullYear().toString().slice(-2), departmentHelper, helper];
    var username = components.join(""); // Generates STU25MCA001
